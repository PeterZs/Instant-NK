...
class NeRFRenderer(nn.Module):
    def __init__(self,
        ...
        if cuda_ray:
            # density grid
            density_grid = torch.zeros([256 + 1] * 3) # +1 because we save values at grid
    ...
    def run(self, rays_o, rays_d, bound, num_steps, upsample_steps, bg_color,frame_id=-1):
        ...
        if frame_id < 0:
            ...
            sigmas, rgbs = self(pts.reshape(B, -1, 3), dirs.reshape(B, -1, 3), bound=bound, frame_id=frame_id)
        ...
        if upsample_steps > 0:
            ...
            if frame_id < 0:
                ...
                new_sigmas, new_rgbs = self(new_pts.reshape(B, -1, 3), new_dirs.reshape(B, -1, 3), bound=bound, frame_id=frame_id )
    ...
    def run_cuda(self, rays_o, rays_d, bound, num_steps, upsample_steps, bg_color,frame_id=-1):
        # rays_o, rays_d: [B, N, 3], assumes B == 1
        # return: image: [B, N, 3], depth: [B, N]
        ...
        if self.training:
            ...
            xyzs, dirs, deltas, rays = raymarching.march_rays_train(rays_o, rays_d, bound, self.density_grid, self.mean_density, self.iter_density, counter, self.mean_count, self.training, 256, False)
            if frame_id < 0:
                sigmas, rgbs = self(xyzs, dirs, bound=bound)
            else:
                sigmas, rgbs = self(xyzs, dirs, bound=bound,frame_id=frame_id)
            depth, image = raymarching.composite_rays_train(sigmas, rgbs, deltas, rays, bound, bg_color)

        else:

            # xyzs, dirs, deltas, rays = raymarching.march_rays_train(rays_o, rays_d, bound, self.density_grid, self.mean_density, self.iter_density, None, self.mean_count, self.training, 256, True)
            # sigmas, rgbs = self(xyzs, dirs, bound=bound)
            # depth, image = raymarching.composite_rays_train(sigmas, rgbs, deltas, rays, bound, bg_color)
            ...
            while step < 1024: # max step
                ...
                xyzs, dirs, deltas = raymarching.march_rays(n_alive, n_step, rays_alive[i % 2], rays_t[i % 2], rays_o, rays_d, bound, self.density_grid, self.mean_density, near, far, 256)
                if frame_id < 0:
                    sigmas, rgbs = self(xyzs, dirs, bound=bound)
                else:
                    sigmas, rgbs = self(xyzs, dirs, bound=bound, frame_id=frame_id)
    ...
    def update_extra_state(self, bound, decay=0.95):
        ...
        X = torch.linspace(-bound, bound, resolution).split(256)
        Y = torch.linspace(-bound, bound, resolution).split(256)
        Z = torch.linspace(-bound, bound, resolution).split(256)
            ...
            for xi, xs in enumerate(X):
                for yi, ys in enumerate(Y):
                    for zi, zs in enumerate(Z):
                        ...
                        tmp_grid[xi * 256: xi * 256 + lx, yi * 256: yi * 256 + ly, zi * 256: zi * 256 + lz] = density
    ...
    def render(self, rays_o, rays_d, bound=1, num_steps=256, upsample_steps=256, staged=False, max_ray_batch=4096*3, bg_color=None, frame_id=-1, knn_field = None, weight_field = None, flag_field = None, res = None, left_top = None,leng = None, ed_nodes = None, ed_motions = None, warp_field = None,smpl_vertices = None, body_params = None, pack = None, **kwargs):
        # rays_o, rays_d: [B, N, 3], assumes B == 1
        # return: pred_rgb: [B, N, 3]
        self.smpl_vertices = smpl_vertices
        ...
        self.leng = leng
        self.ed_nodes = ed_nodes
        self.ed_motions = ed_motions
        ...
        if staged and not self.cuda_ray:
            ...
            for b in range(B):
                ...
                while head < N:
                    ...
                    depth_, image_ = _run(rays_o[b:b+1, head:tail], rays_d[b:b+1, head:tail], bound, num_steps, upsample_steps, bg_color,frame_id)
                    depth[b:b+1, head:tail] = depth_
                    image[b:b+1, head:tail] = image_
                    head += max_ray_batch
        else:
            depth, image = _run(rays_o, rays_d, bound, num_steps, upsample_steps, bg_color, frame_id)
